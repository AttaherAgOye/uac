...
<section class="relative min-h-screen flex items-center overflow-hidden" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
    ...
    <div class="absolute inset-0">
        <img src="{{ asset('images/pages/hi/hero.jpeg') }}" alt="UAC-IOD" class="w-full h-full object-cover scale-105">
        ...
        <div class="grid lg:grid-cols-12 gap-12 items-center">
            ...
            <div class="lg:col-span-7">
                ...
                <div :class="['transition-all duration-700', loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6']">
                    <span class="inline-flex items-center gap-2 px-5 py-2 glass rounded-full text-sm font-medium text-white/90 mb-8">
                ...
                {{-- Heading --}}
                <div :class="['transition-all duration-700 delay-150', loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
                    <h1 class="text-5xl sm:text-6xl lg:text-8xl font-extrabold text-white leading-[0.9] mb-3 tracking-tight">
                        UAC<span class="text-accent">-</span>IOD
                    </h1>
                    <div class="h-1 w-24 bg-gradient-to-r from-accent to-accent-dark rounded-full mb-6"></div>
                    <p class="text-xl sm:text-2xl text-white/60 font-light mb-2">Groupe Industriel</p>
                </div>
                ...
                <div :class="['transition-all duration-700 delay-300', loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
                    <p class="text-lg text-white/70 leading-relaxed mb-10 max-w-xl">
                        Nous opérons dans des secteurs clés — nutrition animale, transport & logistique, vente de véhicules et distribution d'hydrocarbures — avec une vision claire de performance et de durabilité.
                    </p>
                </div>

                {{-- CTAs --}}
                <div class="flex flex-wrap gap-4" :class="['transition-all duration-700 delay-[450ms]', loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
                    <a href="{{ route('groupe') }}"
                       class="group px-8 py-4 bg-accent hover:bg-accent-dark text-primary-dark font-bold rounded-2xl transition-all duration-300 hover:shadow-2xl hover:shadow-accent/30 hover:-translate-y-1 inline-flex items-center gap-3">
                        <span>Découvrir le Groupe</span>
                        <svg class="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                    </a>
                    <a href="{{ route('poles') }}"
                       class="px-8 py-4 glass hover:bg-white/15 text-white font-semibold rounded-2xl transition-all duration-300 hover:-translate-y-1">
            ...
            <div class="lg:col-span-5 hidden lg:block">
                <div class="relative" :class="['transition-all duration-1000 delay-500', loaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12']">
                    {{-- Main stat card --}}
                    <div class="glass rounded-3xl p-8 mb-4">
                        <div class="grid grid-cols-2 gap-6">
                            @php
                                $stats = [
                                    ['value' => '4', 'label' => 'Pôles d\'activités', 'icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
                            ...
                            @foreach($stats as $i => $stat)
                                <div class="text-center p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors duration-300">
                                    <svg class="w-6 h-6 text-accent mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="{{ $stat['icon'] }}"/></svg>
                                    <span class="block text-2xl font-bold text-white">{{ $stat['value'] }}</span>
                                    <span class="text-xs text-white/50 mt-1">{{ $stat['label'] }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    {{-- Accent decoration --}}
                    <div class="absolute -top-4 -right-4 w-24 h-24 border-2 border-accent/20 rounded-3xl -z-10"></div>
                    <div class="absolute -bottom-3 -left-3 w-16 h-16 bg-accent/10 rounded-2xl -z-10 blur-sm"></div>
                </div>
            </div>
        </div>
    </div>
...
{{-- ═══════════════════════════════════════════════════════ --}}
<section id="about" class="py-28 bg-white relative overflow-hidden" x-data="{ v: false }" x-intersect:enter="v = true">
    {{-- Subtle background pattern --}}
    <div class="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-gray-50 to-transparent"></div>
    <div class="absolute bottom-0 left-1/4 w-64 h-64 bg-primary/3 rounded-full blur-3xl"></div>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="grid lg:grid-cols-2 gap-20 items-center">
            {{-- Left: Image composition --}}
            <div :class="['transition-all duration-1000 ease-out', v ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12']">
                <div class="relative">
                    {{-- Main image --}}
                    <div class="rounded-3xl overflow-hidden shadow-2xl">
                        <img src="{{ asset('images/pages/img2.jpeg') }}" alt="UAC-IOD Équipe" class="w-full h-[480px] object-cover">
                    </div>
                    {{-- Floating stat card --}}
                    <div class="absolute -bottom-8 -right-8 bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
                        <div class="flex items-center gap-4">
                            <div class="w-14 h-14 rounded-xl bg-accent/10 flex items-center justify-center">
                                <svg class="w-7 h-7 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                            </div>
                            <div>
                                <span class="text-3xl font-extrabold text-primary-dark">10+</span>
                                <span class="block text-sm text-gray-500">Années d'expérience</span>
                            </div>
                        </div>
                    </div>
                    {{-- Decorative elements --}}
                    <div class="absolute -top-6 -left-6 w-28 h-28 border-2 border-primary/15 rounded-3xl"></div>
                    <div class="absolute top-8 -left-3 w-6 h-6 bg-accent/30 rounded-full"></div>
                </div>
            </div>

            {{-- Right: Content --}}
            <div :class="['transition-all duration-1000 ease-out delay-200', v ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12']">
                ...
                {{-- Quote --}}
                <div class="relative p-6 bg-gradient-to-r from-primary/5 to-transparent rounded-2xl border-l-4 border-accent mb-10">
                    <svg class="absolute top-4 right-4 w-8 h-8 text-primary/10" fill="currentColor" viewBox="0 0 24 24"><path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10H14.017zM0 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151C7.546 6.068 5.983 8.789 5.983 11H10v10H0z"/></svg>
...
<section class="py-28 bg-gray-50 relative overflow-hidden" x-data="{ v: false }" x-intersect:enter="v = true">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {{-- Section header --}}
        <div class="text-center mb-20" :class="['transition-all duration-700', v ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
            <span class="inline-flex items-center gap-2 text-accent font-semibold text-sm uppercase tracking-widest mb-4">
                <span class="w-8 h-px bg-accent"></span>
                Nos activités
                <span class="w-8 h-px bg-accent"></span>
            </span>
            <h2 class="text-4xl sm:text-5xl font-extrabold text-gray-900 mb-4">Nos Pôles d'Activités</h2>
            <p class="text-gray-500 text-lg max-w-2xl mx-auto">Quatre secteurs complémentaires au service du développement économique du Mali.</p>
        </div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            @foreach($poles as $index => $pole)
                <a href="{{ route($pole['route']) }}"
                   class="group relative bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 border border-gray-100"
                   ...
                   style="transition-delay: {{ ($index + 1) * 120 }}ms">
                    ...
                    <div class="h-44 overflow-hidden relative">
                        ...
                        <div class="absolute bottom-3 left-4">
                            <div class="w-11 h-11 rounded-xl bg-gradient-to-br {{ $pole['gradient'] }} flex items-center justify-center shadow-lg">
                    ...
                    {{-- Card content --}}
                    <div class="p-6">
                        <span class="text-xs font-semibold {{ $pole['text'] }} uppercase tracking-wider">{{ $pole['subtitle'] }}</span>
                        <h3 class="text-lg font-bold text-gray-900 mt-1 mb-2 group-hover:text-primary transition-colors">{{ $pole['title'] }}</h3>
                        <p class="text-gray-500 text-sm leading-relaxed">{{ $pole['desc'] }}</p>
                        {{-- Arrow --}}
...
{{-- ═══════════════════════════════════════════════════════ --}}
<section class="py-28 bg-primary-dark relative overflow-hidden" x-data="{ v: false }" x-intersect:enter="v = true">
    {{-- Background effects --}}
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
...
{{-- ═══════════════════════════════════════════════════════ --}}
<section class="py-28 bg-white overflow-hidden" x-data="{ v: false }" x-intersect:enter="v = true">
...
<section class="py-28 bg-gray-50" x-data="{ v: false }" x-intersect:enter="v = true">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-14"
            ...
            <div>
                ...
                <h2 class="text-4xl sm:text-5xl font-extrabold text-gray-900">Dernières Nouvelles</h2>
            </div>
            <a href="{{ route('blog') }}" class="group hidden sm:inline-flex items-center gap-2 text-primary font-bold hover:text-primary-light transition-colors">
                Voir tout
                <svg class="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
            </a>
        </div>
...
{{-- ═══════════════════════════════════════════════════════ --}}
<section class="relative py-32 overflow-hidden" x-data="{ v: false }" x-intersect:enter="v = true">
    {{-- Background --}}
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {{-- Decorative circles --}}
        <div class="absolute top-12 left-[20%] w-32 h-32 border border-white/10 rounded-full"></div>
        <div class="absolute bottom-12 right-[15%] w-48 h-48 border border-white/5 rounded-full"></div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <div :class="['transition-all duration-1000', v ? 'opacity-100 scale-100' : 'opacity-0 scale-95']">
            ...
            <h2 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white mb-6 leading-tight">
                Prêt à collaborer<br>avec <span class="text-accent">nous</span> ?
            </h2>
            <p class="text-white/60 text-lg mb-12 max-w-2xl mx-auto leading-relaxed">
